...
public class QueryController extends BaseController {
	 ...
	 * 报价单折扣
	 */
	@RequestMapping("updiscount")
	@ResponseBody
	public Msg updiscount(String orderid,String key){
		Order order = orderService.selectById(orderid);
		if(MySelfUtil.isDouble(key)){
			order.setYl5(key);
			order.setYl6("价格待复核");
	...
	//同意价格
	@RequestMapping("yesdiscount")
	@ResponseBody
	public Msg yesdiscount(String orderid){
		Order order = orderService.selectById(orderid);
		order.setYl6("价格已同意");
	...
	//不同意价格
	@RequestMapping("nodiscount")
	@ResponseBody
	public Msg nodiscount(String orderid){
		Order order = orderService.selectById(orderid);
		order.setYl6("价格被拒接");
		order.setYl5("");
		if(!orderService.update(order)){
