package com.fh.controller.sercar.goods;
...
public class GoodsController extends BaseController {
	...
	public ModelAndView save() throws Exception{
		...
		pd = this.getPageData();
		pd.put("GOODSRC_ID", this.get32UUID());	//主键
		pd.put("CREACTE_TIME", DateUtil.getSdftime().format(new Date()));	//创建时间
		pd.put("YL10", gsid);
	...
	public ModelAndView list(Page page) throws Exception{
		...
		if(!"admin".equals(username)){
			...
			pd.put("YL10", comid);
			page.setPd(pd);
			varList = goodsService.list(page);	//列出Goods列表
			mv.addObject("pd", pd);
		}else{
		...
		mv.setViewName("sercar/goods/goods_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("sercar/goods/goods_edit");
	...
	public ModelAndView exportExcel() throws Exception{
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var1", varOList.get(i).getString("CAIGOURC_ID"));	    //1
			vpd.put("var2", varOList.get(i).getString("NAME"));	    //2
			vpd.put("var3", varOList.get(i).getString("NUMBER"));	    //3
			vpd.put("var4", varOList.get(i).getString("PRICE"));	    //4
			vpd.put("var5", varOList.get(i).getString("CREACTE_TIME"));	    //5
