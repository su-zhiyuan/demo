										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<a class="btn btn-xs btn-success"  onclick="settlementMx('${var.ORDER_ID}','${var.SETTLEMENT_ID}');">结算明细</a>
													<c:if test="${var.STATUS == '待提交' || var.STATUS == '复核未通过' }">
														<a class="btn btn-xs btn-danger" onclick="submit('${var.SETTLEMENT_ID}');">
															提交
														</a>
														<a class="btn btn-xs btn-danger" onclick="del('${var.SETTLEMENT_ID}');">
															删除
														</a>
													</c:if>
												</div>
	...
	<script type="text/javascript">
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>settlement/delete.do?SETTLEMENT_ID="+Id+"&tm="+new Date().getTime();
		...
		//结算明细
		function settlementMx(Id,Id2){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="结算明细";
			 diag.URL = '<%=basePath%>settlement/goSettlementMx.do?ORDER_ID='+Id+'&SETTLEMENT_ID='+Id2;
			 diag.Width = 800;
			 diag.Height = 700;
		...
		function submit(Id) {
			bootbox.confirm("确定要提交吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>order/subsettlement.do?SETTLEMENT_ID="+Id;
					$.get(url,function(data){
						history.go(0);
						nextPage(${page.currentPage});
					});
