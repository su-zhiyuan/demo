package com.fh.controller.sercar.reservations;
...
import com.fh.service.sercar.reservations.ReservationsManager;
...
public class ReservationsController extends BaseController {
	...
	String menuUrl = "reservations/list.do"; //菜单地址(权限用)
	@Resource(name="reservationsService")
	private ReservationsManager reservationsService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Reservations");
		...
		pd.put("RESERVATIONS_ID", this.get32UUID());	//主键
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Reservations");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		reservationsService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Reservations");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		reservationsService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Reservations");
		...
		if(!"admin".equals(username)){
			...
			pd.put("YL10", comid);
			page.setPd(pd);
			varList = reservationsService.list(page);	//列出Reservations列表
			mv.addObject("pd", pd);
		}else{
		...
		mv.setViewName("sercar/reservations/reservations_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		mv.setViewName("sercar/reservations/reservations_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = reservationsService.findById(pd);	//根据ID读取
		mv.setViewName("sercar/reservations/reservations_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Reservations");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			reservationsService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Reservations到excel");
		...
		List<PageData> varOList = reservationsService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var8", varOList.get(i).getString("CREATIONTIME"));	    //8
			...
			vpd.put("var14", varOList.get(i).getString("YL5"));	    //14
			vpd.put("var15", varOList.get(i).getString("YL6"));	    //15
			vpd.put("var16", varOList.get(i).getString("YL7"));	    //16
			vpd.put("var17", varOList.get(i).getString("YL8"));	    //17
			vpd.put("var18", varOList.get(i).getString("YL9"));	    //18
			vpd.put("var19", varOList.get(i).getString("YL10"));	    //19
			varList.add(vpd);
		}
