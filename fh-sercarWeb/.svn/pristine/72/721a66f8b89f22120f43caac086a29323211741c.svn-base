...
<body class="no-skin">
					...
					<div class="row">
						...
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="sex" id="id" data-placeholder="请选择" style="vertical-align:top;width: 120px;">
						...
						<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
							<thead>
								<tr>
									...
									<!-- <th class="center">所属公司ID</th> -->
									<th class="center">所属部门</th>
									<th class="center">简历路径</th>
							...
							<c:choose>
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											<td class='center'>
												<label class="pos-rel"><input type='checkbox' name='ids' value="${var.USERINFO_ID}" class="ace" /><span class="lbl"></span></label>
											</td>
											<td class='center' style="width: 30px;">${vs.index+1}</td>
											<%-- <td class='center'>${var.USERNAME}</td> --%>
											<td class='center'>${var.NAME}</td>
											<td class='center'>${var.SEX}</td>
											<td class="center"><a title="发送电子邮件" style="text-decoration:none;cursor:pointer;" <c:if test="${QX.email == 1 }">onclick="sendEmail('${var.YL2 }');"</c:if>>${var.YL2 }&nbsp;<i class="ace-icon fa fa-envelope-o"></i></a></td>
											<%-- <td class='center'>${var.CARD_ID}</td> --%>
											<td class='center'>${var.TEL}</td>
											<%-- <td class='center'>${var.ADDR}</td> --%>
											<td class='center'>${var.ROLETYPE}</td>
											<td class='center'>${var.STATUS}</td>
											<%-- <td class='center'>${var.YL1}</td> --%>
											<td class='center'>${var.YL3}</td>
											<td class='center'><a href="${var.YL4}">${var.YL4}</a></td>
											<td class='center'>${var.REMARK}</td>
											<td class='center'>
											<c:if test="${var.YL6=='使用'}">使用</c:if>
											<c:if test="${var.YL6=='停用'}">停用</c:if>
											</td>
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="编辑" onclick="edit('${var.USERINFO_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" title="编辑"></i>
													</a>
													</c:if>
													<%-- <c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" onclick="del('${var.USERINFO_ID}');">
														<i class="ace-icon fa fa-trash-o bigger-120" title="删除"></i>
													</a>
													</c:if> --%>
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" title="简历" onclick="jianli('${var.USERINFO_ID}');">简历</a>
													</c:if>
												</div>
	...
	<script type="text/javascript">
		...
		//简历
		function jianli(USERINFO_ID){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="简历";
			 diag.URL = '<%=basePath%>userinfo/gojianli.do?USERINFO_ID='+USERINFO_ID;
			 diag.Width = 800;
			 diag.Height = 1000;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
		...
		//去发送电子邮件页面
		function sendEmail(YL2){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="发送电子邮件";
			 diag.URL = '<%=basePath%>head/goSendEmail.do?EMAIL='+YL2;
			 diag.Width = 660;
			 diag.Height = 486;
			 diag.CancelEvent = function(){ //关闭事件
				diag.close();
