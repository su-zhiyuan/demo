package com.fh.controller.sercar.rescue;
...
import com.fh.service.sercar.rescue.RescueManager;
...
public class RescueController extends BaseController {
	...
	String menuUrl = "rescue/list.do"; //菜单地址(权限用)
	@Resource(name="rescueService")
	private RescueManager rescueService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Rescue");
		...
		pd.put("RESCUE_ID", this.get32UUID());	//主键
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Rescue");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		rescueService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Rescue");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		rescueService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Rescue");
		...
		if(!"admin".equals(username)){
			pd = this.getPageData();
			pd.put("BIANMA", "jiuyuanzhuangtai");//获取数据字典的字节码
			...
			pd.put("YL10", comid);
			page.setPd(pd);
			varList = rescueService.list(page);	//列出Rescue列表
			mv.addObject("pd", pd);
		}else {
			pd2 = this.getPageData();
			pd2.put("BIANMA", "jiuyuanzhuangtai");//获取数据字典的字节码
			...
			if(pd2.getString("lastStart")==null && pd2.getString("lastEnd")==null 
					&& pd.getString("keywords")==null && pd2.getString("name")==null){
				SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		...
		mv.setViewName("sercar/rescue/rescue_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		mv.setViewName("sercar/rescue/rescue_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = rescueService.findById(pd);	//根据ID读取
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Rescue");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			rescueService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Rescue到excel");
		...
		List<PageData> varOList = rescueService.listAll(pd);
		List<PageData> varList = new ArrayList<PageData>();
		for(int i=0;i<varOList.size();i++){
			PageData vpd = new PageData();
			vpd.put("var1", varOList.get(i).getString("ORDER_ID"));	    //1
			vpd.put("var2", varOList.get(i).getString("CONTACTS"));	    //2
			vpd.put("var3", varOList.get(i).getString("CONTACTSTEL"));	    //3
			vpd.put("var4", varOList.get(i).getString("STATE"));	    //4
			vpd.put("var5", varOList.get(i).getString("CREATER"));	    //5
			vpd.put("var6", varOList.get(i).getString("CREATIONTIME"));	    //6
