package com.fh.controller.sercar.complaint;
...
import com.fh.service.sercar.complaint.ComplaintManager;
...
public class ComplaintController extends BaseController {
	...
	String menuUrl = "complaint/list.do"; //菜单地址(权限用)
	@Resource(name="complaintService")
	private ComplaintManager complaintService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Complaint");
		...
		pd = this.getPageData();
		pd.put("COMPLAINT_ID", this.get32UUID());	//主键
		pd.put("CREATER_BY", Jurisdiction.getUsername());//创建人
		pd.put("COMPLAINT_TIME", DateUtil.getSdftime().format(new Date()));//投诉时间
		pd.put("YL10", gsid);
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Complaint");
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Complaint");
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Complaint");
		...
		if(!"admin".equals(username)){
			pd = this.getPageData();		
			pd.put("BIANMA", "tsjyzhuangtai");//获取数据字典的字节码
			...
			pd.put("YL10", comid);
			page.setPd(pd);
			varList = complaintService.list(page);	//列出Complaint列表
			mv.addObject("pd", pd);
		}else{
			pd2 = this.getPageData();		
			pd2.put("BIANMA", "tsjyzhuangtai");//获取数据字典的字节码
		...
		mv.setViewName("sercar/complaint/complaint_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		mv.setViewName("sercar/complaint/complaint_edit");
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Complaint");
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Complaint到excel");
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var1", varOList.get(i).getString("CREATER_BY"));	    //1
			vpd.put("var2", varOList.get(i).getString("COMPLAINT_NAME"));	    //2
			vpd.put("var3", varOList.get(i).getString("COMPLAINT_TEL"));	    //3
			vpd.put("var4", varOList.get(i).getString("COMPLAINT_CONTENT"));	    //4
			vpd.put("var5", varOList.get(i).getString("COMPLAINT_TIME"));	    //5
