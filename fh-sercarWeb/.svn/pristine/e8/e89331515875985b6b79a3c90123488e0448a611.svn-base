...
<body class="no-skin">
			...
			<div class="main-content-inner">
				<div class="page-content">
					<div class="row">
						...
						<!-- 检索  -->
						<form action="reception/list.do" method="post" name="Form" id="Form">
						<table style="margin-top:5px;">
							<tr>
								...
								<td style="vertical-align:top;padding-left:2px;">
								 	<select class="chosen-select form-control" name="is_agree" id="id" data-placeholder="请选择" style="vertical-align:top;width: 120px;">
										<option value="">--是否同意路试--</option>
										<option value="是" <c:if test="${'是'==pd.is_agree}">selected</c:if> > 是</option>
										<option value="否" <c:if test="${'否'==pd.is_agree}">selected</c:if> > 否</option>
								  	</select>
								</td>
								<c:if test="${QX.cha == 1 }">
								...
								<c:when test="${not empty varList}">
									...
									<c:forEach items="${varList}" var="var" varStatus="vs">
										<tr>
											...
											<td class='center'>${var.SERVICE_ADVISOR}</td>
											<td class='center'>${var.MILEAGE}</td>
											<td class='center'>${var.STATUS}</td>
											<td class='center'>${var.IS_AGREE}</td>
											<td class="center">
												...
												<c:if test="${var.STATUS == '待提交' || var.STATUS == '复核未通过' || var.STATUS == '客户已拒绝'}">
													<a class="btn btn-xs btn-success" title="修改" onclick="submit('${var.RECEPTION_ID}');">
														提交复核
													</a>
	...
	<script type="text/javascript">
		...
		function details(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>reception/goDetails.do?RECEPTION_ID='+Id;
			 diag.Width = 550;
			 diag.Height = 455;
		...
		function submit(Id) {
			bootbox.confirm("确定要提交吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>order/subreception.do?RECEPTION_ID="+Id;
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>reception/goEdit.do?RECEPTION_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>reception/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
