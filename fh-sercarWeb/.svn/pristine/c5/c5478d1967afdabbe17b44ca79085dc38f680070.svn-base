package com.fh.controller.sercar.proclamation;
...
import com.fh.service.sercar.proclamation.ProclamationManager;
...
public class ProclamationController extends BaseController {
	...
	String menuUrl = "proclamation/list.do"; //菜单地址(权限用)
	@Resource(name="proclamationService")
	private ProclamationManager proclamationService;
	...
	public ModelAndView save() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"新增Proclamation");
	...
	public void delete(PrintWriter out) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"删除Proclamation");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "del")){return;} //校验权限
		PageData pd = new PageData();
		pd = this.getPageData();
		proclamationService.delete(pd);
		out.write("success");
		out.close();
	}
	...
	public ModelAndView edit() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"修改Proclamation");
		if(!Jurisdiction.buttonJurisdiction(menuUrl, "edit")){return null;} //校验权限
		ModelAndView mv = this.getModelAndView();
		PageData pd = new PageData();
		pd = this.getPageData();
		proclamationService.edit(pd);
		mv.addObject("msg","success");
		mv.setViewName("save_result");
		return mv;
	}
	...
	public ModelAndView list(Page page) throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"列表Proclamation");
		...
		if(!"admin".equals(username)){
			pd = this.getPageData();		
			pd.put("BIANMA", "gonggzhuangtai");//获取数据字典的字节码
			...
			pd.put("BIANMA", "gonggleixing");//获取数据字典的字节码
			...
			pd.put("YL1", comid);
			page.setPd(pd);
			varList = proclamationService.list(page);	//列出Proclamation列表
			mv.addObject("pd", pd);
		}else{			
			pd2 = this.getPageData();		
			pd2.put("BIANMA", "gonggzhuangtai");//获取数据字典的字节码
			...
			pd2.put("BIANMA", "gonggleixing");//获取数据字典的字节码
		...
		mv.setViewName("sercar/proclamation/proclamation_list");
	...
	public ModelAndView goAdd()throws Exception{
		...
		mv.setViewName("sercar/proclamation/proclamation_edit");
	...
	public ModelAndView goEdit()throws Exception{
		...
		PageData pd = new PageData();
		pd = this.getPageData();
		pd = proclamationService.findById(pd);	//根据ID读取
	...
	public Object deleteAll() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"批量删除Proclamation");
		...
		if(null != DATA_IDS && !"".equals(DATA_IDS)){
			...
			proclamationService.deleteAll(ArrayDATA_IDS);
			pd.put("msg", "ok");
		}else{
			pd.put("msg", "no");
		}
	...
	public ModelAndView exportExcel() throws Exception{
		logBefore(logger, Jurisdiction.getUsername()+"导出Proclamation到excel");
		...
		List<PageData> varOList = proclamationService.listAll(pd);
		...
		for(int i=0;i<varOList.size();i++){
			...
			vpd.put("var5", varOList.get(i).getString("CREATE_BY"));	    //5
