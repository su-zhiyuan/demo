...
<body class="no-skin">
					...
					<div class="row">
						...
						<form action="partoffer/list.do" method="post" name="Form" id="Form">
							<table id="simple-table" class="table table-striped table-bordered table-hover" style="margin-top:5px;">	
								...
								<!-- 开始循环 -->	
								<c:choose>
								<c:when test="${not empty list_inforelay}">
									<c:forEach items="${list_inforelay}" var="inforelay" varStatus="vs">
										<tr>
											...
											<td class='center'>${inforelay.INFORELAY_ID}</td>
											<td class='center'>${inforelay.STATUS}</td>
											<td class='center'>${inforelay.RELAY_CONTEND}</td>
											<td class='center'>
												<c:forEach items="${inforelay.luyinList}" var="luyin" varStatus="vsp" >
											...
											<td class="center">
												<a class="btn btn-xs btn-success" title="复核"  onclick="goInforelay('${inforelay.INFORELAY_ID}');">
													复核
												</a>
											</td>
										</tr>
	...
	<script type="text/javascript">
		...
		//去复核信息反馈单
		function goInforelay(id){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>recheck/goInforelay.do?INFORELAY_ID='+id;
			 diag.Width = 450;
			 diag.Height = 355;
