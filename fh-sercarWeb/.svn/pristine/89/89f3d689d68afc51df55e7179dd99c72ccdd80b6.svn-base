...
<body class="no-skin">
										...
										<tr>
											...
											<td class="center">
												...
												<div class="hidden-sm hidden-xs btn-group">
													<c:if test="${QX.add == 1 }">
														<a class="btn btn-mini btn-success" style="margin-right:3px; height:30px;line-height:30px; margin-bottom:5px;" onclick="purchaseMx('${var.ORDER_ID}','${var.PURCHASE_ID}','${var.DELIVERY_TIME}');">+采购明细</a>
													</c:if>
													<c:if test="${QX.edit == 1 }">
													<a class="btn btn-xs btn-success" style="margin-right:3px; width:30px; height:30px;line-height:30px; margin-bottom:5px;position:relative;" title="编辑" onclick="edit('${var.PURCHASE_ID}');">
														<i class="ace-icon fa fa-pencil-square-o bigger-120" style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);" title="编辑"></i>
													</a>
													
													</c:if>
													<c:if test="${QX.del == 1 }">
													<a class="btn btn-xs btn-danger" style="margin-right:3px; width:30px; height:30px;line-height:30px; margin-bottom:5px;position:relative;" onclick="del('${var.PURCHASE_ID}');">
	...
	<script type="text/javascript">
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>purchase/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		//采购明细
		function purchaseMx(id1,id2,str){
			 top.jzts();
			 var diag = new top.Dialog();
			 diag.Drag=true;
			 diag.Title ="采购明细";
			 diag.URL = '<%=basePath%>purchase/goPurchaseMx.do?ORDER_ID='+id1+'&PURCHASE_ID='+id2+'&DELIVERY_TIME='+str;
			 diag.Width = 800;
			 diag.Height = 700;
			 diag.Modal = true;				//有无遮罩窗口
			 diag. ShowMaxButton = true;	//最大化按钮
